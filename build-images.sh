...
password_pusher_version="1.63"
# Create a new empty container image
container=$(buildah from scratch)

# Reuse existing nodebuilder-passwordpusher container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-passwordpusher; then
	echo "Pulling NodeJS runtime..."
	buildah from --name nodebuilder-passwordpusher -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
	...
	--label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/pglombardo/pwpush:$password_pusher_version" \
	"${container}"
# Commit the image
